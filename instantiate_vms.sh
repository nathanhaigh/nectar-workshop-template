...
usage="USAGE: $(basename $0) [-h] [-d] -i <image id> [-p <prefix>] [-u <script>] [-n <number of VMs>] [-f <int>] [-s <int>] [-x <string>] -k <ssh key> -g <security group> [-c <cell>]
  where:
...
while getopts ":hdi:p:u:n:x:k:f:s:g:c:" opt; do
case $opt in
    h) echo "$usage"
       exit
       ;;
...
for i in `seq --format="${FORMAT}" ${FIRST} ${LAST}`; do
  ...
  if [[ ! $(nova list --name "^${INSTANCE_NAME}$" | fgrep ${INSTANCE_NAME}) ]]; then
    ...
    (( n_booted++ ))
    if [[ ${DRYRUN} == 0 ]]; then
        nova boot --hint cell=${CELL} --flavor=${FLAVOUR} --image=${IMAGE_ID} ${INSTANCE_NAME} --security-groups=${SECURITY_GROUP} --key-name=${SSH_KEY_NAME} --user-data=${USER_DATA_FILE} --meta description="${VM_NAME_PREFIX}${i}" --meta creator='Nathan S. Watson-Haigh'
    fi
  else
